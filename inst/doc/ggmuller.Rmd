...
In the population plot, the white space is background, and as before the common ancestor is olive brown (in the default palette).
...
Of course there are more concise ways to create this data frame, but the long-winded version enables the data clearer in the code. Note the inclusion of an optional “Fitness” column containing the fitness values. We combine the data using `get_Muller_df` as before:

```{r}
Muller_df3 <- get_Muller_df(edges3, pop3)
```

We create two versions of the plot. The first version is coloured by genotype identity (the default):

```{r}
Muller_plot(Muller_df3, add_legend = TRUE)
```

The second version is (using the RColorBrewer package) coloured by fitness, making use of that optional column:
...
You can change the `Muller_plot` or `Muller_pop_plot` axis labels using the options `xlab` and `ylab`.
